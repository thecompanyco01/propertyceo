...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>${BASE}/buy</loc><lastmod>${DATE}</lastmod><changefreq>weekly</changefreq><priority>1.0</priority></url>
  ...
  <url><loc>${BASE}/compare/</loc><lastmod>${DATE}</lastmod><changefreq>weekly</changefreq><priority>0.8</priority></url>
XMLHEAD
...
for f in compare/*.html; do
  [ ! -f "$f" ] && continue
  slug=$(basename "$f" .html)
  [ "$slug" = "index" ] && continue
  echo "  <url><loc>${BASE}/compare/${slug}</loc><lastmod>${DATE}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>" >> sitemap.xml
done

# Eviction guides
for f in eviction/*.html; do
  [ ! -f "$f" ] && continue
  slug=$(basename "$f" .html)
  [ "$slug" = "index" ] && continue
  echo "  <url><loc>${BASE}/eviction/${slug}</loc><lastmod>${DATE}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>" >> sitemap.xml
done

# Calculator pages
for f in calculators/*.html; do
  [ ! -f "$f" ] && continue
  slug=$(basename "$f" .html)
  echo "  <url><loc>${BASE}/calculators/${slug}</loc><lastmod>${DATE}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>" >> sitemap.xml
done
